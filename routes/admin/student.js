...
function showData(res) {
    User.find({
        ...
        if (err) throw err;
        if (result.length != 0) {
            res.render('admin/student', {student: result, error: error});
        } else {
            if (error == 0) {
                error = 4;
            }
            res.render('admin/student', {student: '', error: error});
        }
    });
}
...
router.get('/:username', function (req, res) {
    User.find({username: req.param('username')}, function (err, result) {
        if (err) throw err;
        res.render('admin/gestion_student', {student: result, error: error});
    });
});
