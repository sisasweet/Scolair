    ...
    Module.find().populate("id_ens", "name").exec(function (err, result1) {
        ...
        if (result1.length != 0) {
            User.find({
                ...
                if (err) throw err;
                if (result2.length != 0) {
                    res.render('admin/module', {module: result1, enseignant: result2, error: error});
                } else {
                    res.render('admin/module', {module: result1, enseignant: '', error: error});
                }
            });
        } else {
            error = 4;
            User.find({
                    ...
                    if (err) throw err;
                    if (result2.length != 0) {
                        res.render('admin/module', {module: '', enseignant: result2, error: error});
                    } else {
                        res.render('admin/module', {module: '', enseignant: '', error: error});
                    }
                }
            );
        }
...
router.get('/:module', function (req, res) {
    Module.find({name: req.param('module')}, function (err, result) {
        ...
        User.find({enseignant: true}, function (err, result2) {
            if (err) throw err;
            res.render('admin/gestion_module', {module: result, enseignant: result2, error: error});
        })
    });
});
